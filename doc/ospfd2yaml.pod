...
B<ospfd2yaml> - convert OpenBSD B<ospfd> or B<ospf6d> LSDB to YAML file
...
B<ospfd2yaml> takes the OSPF link state database of the OpenBSD
...
If necessary B<sudo> is used for calling B<ospfctl> or B<ospf6ctl>
to gain privileges.
...
This option may only be used with B<-6>.
